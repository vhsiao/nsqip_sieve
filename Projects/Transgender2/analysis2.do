...
do initiate_excel "`excel_file'" "prediction" "Variable Observation OR_any_complication_lifethreaten p"
...
local sites top_surgery ftm_top_e ftm_internal_e ftm_bottom_e mtf_top_e head_neck_e
foreach site of varlist `sites' {
	replace `site' = 0 if multisite_e
}
...
generate unplanned_readmission_e = unplannedreadmission1_e == 1 & readmission1_e ==1
generate unplanned_reop_or_readm_e = unplanned_readmission_e==1 | reoperation1_e ==1 
...
local complications new_sssi_e new_dssi_e new_ossi_e ///
	...
	sepsis_septic_shock mi_cardiac_arrest_cva failwean_reintub death_e unplanned_readmission_e reoperation1_e
...
do compound_variable "any_complication_lifethreaten" "`complications'"
...
local complications `complications' any_complication_lifethreaten
...
quietly {
	...
	label variable unplanned_readmission_e "Unplanned Readmission"
	label variable reoperation1_e "Unplanned Reoperation"
	label variable unplanned_reop_or_readm_e "Unplanned Readmission or Reoperation"
	label variable any_complication_lifethreaten "Any Complication"
}
...
do put_all_in_excel "`excel_file'" "prediction" 2 1 "logistic" "`predictors'" any_complication_lifethreaten "independent"
...
quietly {
	...
	forvalues i=1/7 {
		...
		foreach dependent_var of varlist `complications' {
			...
			local percent_val_`j' : display %04.3f `percent_val_`j''
 ...
 */
 ...
 /*
logistic any_complication_lifethreaten age BMI optime ///
		smoke_e diabetes2_e ///
...
*/
logistic any_complication_lifethreaten optime ftm_top_e mtf_top_e mtf_bottom_e multioperation_e
logistic any_complication_lifethreaten optime multioperation_e
